...
bool _hasFreshLegJointTelemetry(RealJoint joint) {
  return MotorEnableSafety.hasFreshLegCanTelemetry(<num>[
...
Future<void> _run() async {
  ...
  _subs.add(
    motorHealth.healthStream.listen((event) {
      ...
      if (arbiter.state is Grounded && motorHealth.hasFaults) {
        _log.info('Grounded with faulted motors — starting recovery');
        unawaited(motorHealth.recoverFaults());
      }
    }),
  );
  ...
  UnifiedCmsServer? cmsServiceForHotplug;

  String? motorEnableBlockReason() {
    final safetyRejection = MotorEnableSafety.blockReason(
      state: arbiter.state,
      position: joint.position,
      velocity: joint.velocity,
      jointLimitRad: _cfg.jointLimitRad,
      hasFreshJointTelemetry: _hasFreshLegJointTelemetry(joint),
      hasUnrecoveredMotorFaults: motorHealth.hasFaults,
      safePose:
          profileManager?.currentProfile.sittingPose ??
          defaultProfile.sittingPose,
      maxSafePoseErrorRad: _cfg.enablePoseToleranceRad,
    );
    return safetyRejection ??
        UnifiedCmsServer.motorEnableJointBlockReason(joint);
  }
  ...
  final motorOutput = MotorOutputController(
    ...
    onOutputChanged: (enabled) {
      controller?.syncMotorOutputEnabled(enabled);
      _log.info('motor output enabled=$enabled');
    },
  );
  motorHealth.attachMotorOutput(motorOutput);
  if (arbiter.state is Grounded && motorHealth.hasFaults) {
    unawaited(motorHealth.recoverFaults());
  }
      ...
      (action) {
        ...
        if (now.difference(lastWheelActionLog).inMilliseconds >= 200) {
          ...
          _log.info(
            ...
            'vel='
            'FR=${velocity.frFoot.toStringAsFixed(2)} '
            'FL=${velocity.flFoot.toStringAsFixed(2)} '
            'RR=${velocity.rrFoot.toStringAsFixed(2)} '
            'RL=${velocity.rlFoot.toStringAsFixed(2)}',
          );
        }
  ...
  if (controller != null) {
    ...
    _controllerHotplugTimer = Timer.periodic(const Duration(seconds: 3), (_) {
      ...
      _controllerHotplugTimer?.cancel();
      controller = xbox;
      xbox.syncMotorOutputEnabled(motorOutput.isEnabled);
      _log.info('Xbox/NC500 hot-plugged: ${_cfg.xboxDevice}');
      controlDog = RealControlDog(
        brain: brain,
        imu: imu,
        joint: joint,
      ...
      profileManager = pm;
      cmsServiceForHotplug?.profileManager = pm;
  ...
  cmsServiceForHotplug = cmsService;
  ...
  Timer.periodic(const Duration(seconds: 10), (_) async {
    ...
    try {
      ...
      if (minV < lowVoltageThreshold) {
        ...
        motorOutput.inhibitEnables(
          'low-voltage lockout: ${minV.toStringAsFixed(1)}V '
          '< ${lowVoltageThreshold.toStringAsFixed(1)}V',
        );
        _log.severe(
...
void _registerShutdown({
  ...
  required MotorOutputController motorOutput,
