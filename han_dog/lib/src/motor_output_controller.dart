...
/// cannot be emitted while a Disable CAN request is in flight.
class MotorOutputController {
  ...
  bool _disableRequestCompleted = true;
  ...
  String? enableRejection(ControlSource source) {
    ...
    if (!_disableRequestCompleted) {
      return 'previous motor Disable request has not completed';
    }
  ...
  Future<String?> enable(ControlSource source) {
    ...
    return _enqueue(() async {
      ...
      try {
        ...
        await _motor.enable();
        _disableRequestCompleted = false;
        if (requestId != _nextRequestId) {
          _setEnabled(false);
          await _bestEffortDisableRequest();
          return 'motor enable superseded by a later request';
        }
        _setEnabled(true);
        return null;
  ...
  /// Closes the software gate and then completes a Disable CAN request.
  Future<void> disable({bool clearErrors = false}) {
    ...
    return _enqueue(() async {
      try {
        await _completeDisableRequest(clearErrors: clearErrors);
      } finally {
        if (requestId == _nextRequestId) {
          _setEnabled(false);
        }
  ...
  /// alone is not sufficient because a Disable CAN frame may still be queued,
  /// in flight, or may have failed. A fresh request must complete
  ...
  Future<T> runWhileDisabled<T>(Future<T> Function() operation) {
    ...
    return _enqueue(() async {
      ...
      await _completeDisableRequest();
      return operation();
    });
  }

  /// Closes the output gate, completes a Disable request, then runs
  ...
  Future<void> _completeDisableRequest({bool clearErrors = false}) async {
    _disableRequestCompleted = false;
    try {
      await _motor.disable(clearErrors: clearErrors);
      _disableRequestCompleted = true;
    } catch (_) {
      _disableRequestCompleted = false;
      rethrow;
    }
  ...
  Future<void> _bestEffortDisableRequest() async {
    try {
      await _completeDisableRequest();
    } catch (_) {
