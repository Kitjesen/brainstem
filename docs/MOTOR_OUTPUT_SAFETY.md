...
- 输出动作门在物理 Disable 发出前立即关闭；物理 Disable 失败时，门保持关闭。
- Enable、Disable 和维护操作均串行执行。
- Enable 前先下发“当前位置保持”目标，再开扭矩；准备期间收到 Disable 时，旧 Enable 会被取消，不能再发 Enable 帧。
- Enable 局部失败时会 best-effort 补发全局 Disable；在物理断扭矩未确认前，所有后续 Enable 都被拒绝。
- 校零、单关节清故障和自动故障恢复都必须先在同一串行队列中确认物理 Disable 成功，不能把“软件门已关”当作“已经断扭矩”。
...
1. CMS 必须为 `Grounded`。
2. 12 个腿部关节反馈频率至少为 50 Hz，位置和速度均为有限数。
...
- `dart test han_dog/ han_dog_brain/ frequency_watch/ skinny_dog_algebra/`：262 项通过。
- 变更文件的 `dart analyze`：零问题。
- 全量 `dart analyze han_dog_brain/ han_dog/`：保留 17 条既有独立工具/命名告警；本次新增的示例接口错误已修复，未新增分析错误。
