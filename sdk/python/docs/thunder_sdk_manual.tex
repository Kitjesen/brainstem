...
\fancyfoot[LE,RO]{\small ORIX SDK v1.0.0}
...
为了使 RL 策略的观测向量中各维度的数值范围保持一致，IMU 陀螺仪和关节速度在输入神经网络前会进行缩放处理。
...
控制权优先级从高到低依次为：YUNZHUO 遥控器（最高优先级）、Xbox 手柄、SDK gRPC（最低优先级）。高优先级控制源活跃时，低优先级的指令将被忽略。
...
# 导出日志到本地（在开发机上执行）
scp sunrise@192.168.66.190:logs/han_dog_*.log ./orix_logs/
\end{lstlisting}
