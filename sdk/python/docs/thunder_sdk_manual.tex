...
近年来，国家高度重视人工智能与机器人教育。《新一代人工智能发展规划》明确要求在各级院校开设人工智能课程，教育部《人工智能教育基地建设指南》进一步推动了 AI+机器人实验室的普及。四足机器人作为融合机械工程、电子控制、传感器技术、强化学习和嵌入式系统的综合平台，正在成为 STEM 教育和高校科研的理想载体。

ORIX 教育版四足机器狗是大算机器人面向教育场景推出的专业级开发平台。设计理念：\textbf{学以致用}（可编程、调参、训练策略的真实机器人）、\textbf{降低门槛}（3 行 Python 代码控制行走）、\textbf{不设天花板}（覆盖本科到研究生的完整学习路径）、\textbf{工业级可靠}（与工业机器人相同的控制架构和安全机制）。

本手册面向高校教师（课程实验设计）、学生开发者（从零上手）、科研人员（算法验证）和竞赛团队（快速原型）。建议首次使用按顺序阅读前三章，后续章节按需查阅。
...
\textbf{注意：}以上为概念性分解，实际观测向量的精确组成可能因策略版本不同而略有差异。
...
\midrule
\endhead
低电压 ($<$ 18V) & \texttt{get\_voltage()} 返回值低于 42 \\
