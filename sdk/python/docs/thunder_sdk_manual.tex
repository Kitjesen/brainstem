...
以下是 ORIX 全部 16 个关节的编号和 URDF 名称。编写底层控制或进行运动学分析时需要频繁参考此表。具体的运动范围请参考 URDF 文件或通过 SDK 接口查询。
...
3 & FR Foot & 前右 & 脚轮 (连续) & \texttt{fr\_foot\_joint} \\
4 & FL Hip & 前左 & 髋 (横滚) & \texttt{fl\_hip\_joint} \\
5 & FL Thigh & 前左 & 大腿 (俯仰) & \texttt{fl\_thigh\_joint} \\
6 & FL Calf & 前左 & 小腿 (俯仰) & \texttt{fl\_calf\_joint} \\
7 & FL Foot & 前左 & 脚轮 (连续) & \texttt{fl\_foot\_joint} \\
8 & RR Hip & 后右 & 髋 (横滚) & \texttt{rr\_hip\_joint} \\
9 & RR Thigh & 后右 & 大腿 (俯仰) & \texttt{rr\_thigh\_joint} \\
10 & RR Calf & 后右 & 小腿 (俯仰) & \texttt{rr\_calf\_joint} \\
11 & RR Foot & 后右 & 脚轮 (连续) & \texttt{rr\_foot\_joint} \\
12 & RL Hip & 后左 & 髋 (横滚) & \texttt{rl\_hip\_joint} \\
13 & RL Thigh & 后左 & 大腿 (俯仰) & \texttt{rl\_thigh\_joint} \\
14 & RL Calf & 后左 & 小腿 (俯仰) & \texttt{rl\_calf\_joint} \\
15 & RL Foot & 后左 & 脚轮 (连续) & \texttt{rl\_foot\_joint} \\
\bottomrule
\end{longtable}
...
\[
q_{\text{target}} = \text{action\_output} \times \text{actionScale} + \text{policyDefaultPose}
\]

\subsection{站立姿态}

Profile 使用 \texttt{standUpPose} 作为站起、回站和 gesture 的物理目标，使用
\texttt{policyDefaultPose} 作为 observation 和策略动作反归一化的训练零点。
兼容字段 \texttt{standingPose} 在任一显式字段缺失时作为其回退值。站起过程会从
坐姿线性插值到物理站姿，约 3 秒完成。
...
站起过程分为 150 帧，按线性插值平滑过渡：第 0 帧为当前坐姿（sittingPose），第 75 帧为中间过渡态（50\% 位置），第 150 帧为完全物理站立姿态（standUpPose）。
...
中间状态 & Transitioning (StandUp)，从当前行走姿态插值到 standUpPose \\
...
\begin{lstlisting}[style=plain]
{
  ...
  "standUpPose": [0.0, -0.65, 1.5, 0.0, ...],
  "policyDefaultPose": [0.0, -0.65, 1.5, 0.0, ...],
