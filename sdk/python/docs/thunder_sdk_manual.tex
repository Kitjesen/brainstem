...
{\Huge\bfseries\color{thunderblue} ORIX 教育版四足机器狗\\[0.5em] Python SDK 开发手册}
...
{\small\color{thundergray} Copyright \copyright\ 2026 大算机器人. All rights reserved.}
...
本文档介绍 ORIX 教育版四足机器狗的硬件组成、传感器、执行器和通信架构。
...
ORIX 机器人的整体硬件架构以 S100P 主控板（RDK X5, aarch64, Ubuntu 22.04）为核心，通过多种总线连接外围设备。主控板向下分别连接四类子系统：
...
\midrule
\endhead
HI91 IMU & 串口 (UART) & 50Hz 姿态与角速度数据采集 \\
Robstride 电机 (16个) & PCAN CAN 总线 & 4 条腿各 4 个电机，PD 位置/速度控制 \\
SDK 远程控制 & 以太网 (gRPC) & 开发机通过 gRPC over HTTP/2 发送指令 \\
物理遥控器 & SBUS 串口 / USB HID & YUNZHUO 遥控器或 Xbox 手柄，最高优先级 \\
\bottomrule
\end{longtable}

\noindent 电机通过 PCAN CAN 总线分三路连接：前右腿（FR, 4 个电机）、前左腿（FL, 4 个电机）、后两腿（RR+RL, 共 8 个电机）。所有通信均由主控板上的 brainstem 运行时统一调度。
...
ORIX 共有 \textbf{16 个自由度}，分为 4 条腿，每条腿包含 3 个旋转关节和 1 个脚轮：

从俯视角度看，机器人前方左侧为 FL 腿，前方右侧为 FR 腿，后方左侧为 RL 腿，后方右侧为 RR 腿。每条腿从躯干到地面依次包含以下四个关节：
...
\midrule
\endhead
Hip（髋关节） & 横滚方向，控制腿的内外摆动 \\
Thigh（大腿关节） & 俯仰方向，控制腿的前后摆动 \\
Calf（小腿关节） & 俯仰方向，控制膝盖弯曲 \\
Foot（脚轮） & 连续旋转，轮式驱动 \\
\bottomrule
\end{longtable}
...
ORIX 支持两种物理控制器，均通过 \texttt{ControlArbiter} 仲裁控制权：
...
控制链路的执行顺序为：（1）IMU 采样 $\to$（2）构建观测向量 $\to$（3）ONNX 神经网络推理 $\to$（4）PD 力矩计算 $\to$（5）CAN 总线发送指令 $\to$（6）电机执行。以上全部步骤在每个 20ms 周期内完成。
...
电机的生命周期如下：上电后电机默认处于禁用状态。调用 \texttt{enable()} 使能电机后，可以执行 \texttt{stand\_up()}、\texttt{walk()}、\texttt{sit\_down()} 等运动操作。在任何使能状态下调用 \texttt{disable()} 都会立即释放所有电机力矩，电机回到禁用状态。最终下电关闭电源。
...
低压保护触发后，系统依次执行以下步骤：（1）检测到低压（低于 42V）$\to$（2）自动发送坐下指令 $\to$（3）等待机器人进入 Grounded 状态 $\to$（4）禁用全部电机。
...
\begin{enumerate}[nosep]
  \item 检测到任一电机母线电压低于 42V
  \item 自动发送 SitDown 指令
  \item 等待机器人进入 Grounded 状态（安全着地）
  \item 禁用全部电机（Disable）
  \item 日志输出：\texttt{LOW VOLTAGE: xx.xV < 42.0V -- graceful sitDown}
\end{enumerate}
...
\begin{enumerate}[nosep]
  \item \textbf{机器人摔倒}：首先用 \texttt{get\_voltage()} 检查电压。低于 42V 说明是低压保护触发；电压正常则检查关节限位是否超限。
